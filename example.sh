...
oc wait --for=condition=available=true deploy/zz --timeout=60s -n $ZZ_APP_STAGE

STAGE_URL=$(oc get route zz -o template --template='{{.spec.host}}' -n $ZZ_APP_STAGE)

curl http://$STAGE_URL/hello
